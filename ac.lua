...
local storage_file =
minetest.get_worldpath() .. "/ac_data.mt"
...
local data =  
    minetest.deserialize(file:read("*all"))  
...
local created =  
    content:match("created%s*=%s*(%d+)")  

if not created then return 0 end  

return math.floor(  
    (os.time() - tonumber(created)) / 86400  
)
...
if active_players[name] then  
    ...
    local hours =  
        active_players[name].session_time  
        / 3600  
...
func = function(name)  
    ...
    for pname, sdata in pairs(active_players) do  
        ...
        local player_obj =  
            minetest.get_player_by_name(pname)  
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
    ...
    if fields.guardian then
        ...
        for name, data in pairs(persistent) do
            ...
            local rank =
                ranks and ranks.get_rank
                and ranks.get_rank(name)
                or ""

            rank = tostring(rank):lower()

            if rank:find("build10")
              or rank:find("builder10") then

                local total_hours =
                    data.total_hours or 0
                ...
                table.insert(list, {
                  name = name,
                  hours = total_hours, -- REAL
                  h = hours_prog,
                  a = activity_prog,
                  j = join_prog,
                  ok =hours_ok and activity_ok and join_ok,
                      days = days_played
                })
            end
        end
